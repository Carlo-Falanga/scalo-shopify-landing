...
Landing page di conversione per il servizio di migrazione e replatforming su Shopify Plus di un'agenzia
e-commerce. Una sola call to action: prenotare una call gratuita di 30 minuti tramite il form in fondo alla pagina.

Progetto dimostrativo: l'agenzia, i brand citati in pagina e il caso studio sono inventati.
Stack: Laravel 13, Blade, Tailwind CSS 4, SQLite.
...
```bash
git clone https://github.com/Carlo-Falanga/scalo-shopify-landing.git
cd scalo-shopify-landing
composer install
cp .env.example .env
...
e infine il form. I quattro passi del processo sono la pratica standard di una migrazione.
...
Tailwind perché è già nello scaffold di Laravel e mi permette di lavorare
...
Google Tag Manager e Google Analytics 4 sono su account creati per questo progetto. Il container GTM ha due tag:
...
l'attribuzione. È anche il motivo per cui un evento mandato dal backend avrebbe senso: conterebbe ogni lead salvato, indipendentemente dal browser.
...
- **Serve un'informativa privacy sotto il form?** In una landing reale sì, con il link alla policy dell'agenzia.
  Qui non avevo un testo vero da linkare, quindi ho lasciato solo la riga "Nessuna newsletter"
  che dice all'utente cosa succede dopo.
...
## Cosa manca e cosa farei con tre giorni in più

Il primo pezzo che aggiungerei è l'evento mandato anche dal backend con il Measurement Protocol di GA4:
conta ogni lead salvato anche se l'utente ha rifiutato i cookie o ha uno script blocker, e permette
di confrontare i numeri del browser con quelli del server.
...
5. **Test automatici** sul form: validazione, honeypot, throttle, salvataggio e mail. Sono il modo
   per toccare il codice senza rompere quello che funziona.
